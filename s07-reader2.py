...
tickers = ['BTC-USD', 'ETH-USD', 'BNB-USD', 'ADA-USD', 'SOL1-USD', 'XRP-USD', 
           'DOT1-USD', 'DOGE-USD', 'LINK-USD', 'LTC-USD', 'ALGO-USD', 'VET-USD', 
           'MATIC-USD']
...
while len(tickers) != 0 and attempt <= 5:
    tickers = [j 
              for j in tickers 
              if j not in drop] # removing stocks whose data has been extracted from the ticker list
    for i in range(len(tickers)):
        try:
            temp = pdr.get_data_yahoo(tickers[i],
...
stock_cp.to_csv('output/07_pandas_datareader.csv')